...
describe('IncomeApi', () => {
	describe('create', () => {
		test('should create income successfully without place', async () => {
			const now = new Date()
			...
			const result = await zaim.income.create(params)
		...
		test('should create income successfully with place', async () => {
			const now = new Date()
			...
			expect(result.place?.mode).toBe('income')
	...
	describe('update', () => {
		test('should update income successfully without place', async () => {
			const now = new Date()
			const mockOAuth = new MockOAuthClient()
			const mockResponse = {
				money: {
			...
			mockOAuth.setMockResponse(
				'https://api.zaim.net/v2/home/money/income/11820767',
				mockResponse,
			)
			...
			const result = await zaim.income.update(11820767, params)
		...
		test('should update income successfully with place', async () => {
			const now = new Date()
			const placeModified = new Date('2017-06-28 18:24:51')
			const placeCreated = new Date('2016-12-07 23:37:48')
			const mockOAuth = new MockOAuthClient()
